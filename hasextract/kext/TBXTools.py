...
class TBXTools:
    ...
    def create_project(self, project_name, sl_lang, tl_lang="null", overwrite=False):
        ...
        if (
            project_name != ":momory:"
            and os.path.isfile(project_name)
            and not overwrite
        ):
            ...
            with self.conn:
                self.cur = self.conn.cursor()
                self.cur.execute(
                    "CREATE TABLE configuration(id INTEGER PRIMARY KEY, sl_lang TEXT, tl_lang TEXT)"
                )
                self.cur.execute(
                    "CREATE TABLE sl_corpus(id INTEGER PRIMARY KEY, segment TEXT)"
                )
                self.cur.execute(
                    "CREATE TABLE tl_corpus(id INTEGER PRIMARY KEY, segment TEXT)"
                )
                self.cur.execute(
                    "CREATE TABLE sl_tagged_corpus(id INTEGER PRIMARY KEY, tagged_segment TEXT)"
                )
                self.cur.execute(
                    "CREATE TABLE tl_tagged_corpus(id INTEGER PRIMARY KEY, tagged_segment TEXT)"
                )
                self.cur.execute(
                    "CREATE TABLE sl_stopwords (id INTEGER PRIMARY KEY AUTOINCREMENT, sl_stopword TEXT)"
                )
                self.cur.execute(
                    "CREATE TABLE sl_inner_stopwords (id INTEGER PRIMARY KEY AUTOINCREMENT, sl_inner_stopword TEXT)"
                )
                self.cur.execute(
                    "CREATE TABLE tl_stopwords (id INTEGER PRIMARY KEY AUTOINCREMENT, tl_stopword TEXT)"
                )
                self.cur.execute(
                    "CREATE TABLE tl_inner_stopwords (id INTEGER PRIMARY KEY AUTOINCREMENT, tl_inner_stopword TEXT)"
                )
                self.cur.execute(
                    "CREATE TABLE sl_exclusion_regexps (id INTEGER PRIMARY KEY AUTOINCREMENT, sl_exclusion_regexp TEXT)"
                )
                self.cur.execute(
                    "CREATE TABLE tl_exclusion_regexps (id INTEGER PRIMARY KEY AUTOINCREMENT, tl_exclusion_regexp TEXT)"
                )
                self.cur.execute(
                    "CREATE TABLE sl_morphonorm_rules (id INTEGER PRIMARY KEY AUTOINCREMENT, sl_morphonorm_rule TEXT)"
                )
                self.cur.execute(
                    "CREATE TABLE tl_morphonorm_rules (id INTEGER PRIMARY KEY AUTOINCREMENT, tl_morphonorm_rule TEXT)"
                )
                ...
                self.cur.execute(
                    "CREATE TABLE evaluation_terms (id INTEGER PRIMARY KEY AUTOINCREMENT, sl_term TEXT, tl_term TEXT)"
                )
                self.cur.execute(
                    "CREATE TABLE exclusion_terms (id INTEGER PRIMARY KEY AUTOINCREMENT, sl_term TEXT, tl_term TEXT)"
                )
                self.cur.execute(
                    "CREATE TABLE exclusion_noterms (id INTEGER PRIMARY KEY AUTOINCREMENT, sl_term TEXT, tl_term TEXT)"
                )
                self.cur.execute(
                    "CREATE TABLE tokens (id INTEGER PRIMARY KEY AUTOINCREMENT, token TEXT, frequency INTEGER)"
                )
                self.cur.execute(
                    "CREATE TABLE ngrams (id INTEGER PRIMARY KEY AUTOINCREMENT, ngram TEXT, n INTEGER, frequency INTEGER)"
                )
                self.cur.execute(
                    "CREATE TABLE tagged_ngrams (id INTEGER PRIMARY KEY AUTOINCREMENT, ngram TEXT, tagged_ngram TEXT, n INTEGER, frequency INTEGER)"
                )

                self.cur.execute(
                    "CREATE INDEX indextaggedngram on tagged_ngrams (ngram);"
                )

                self.cur.execute(
                    "CREATE TABLE term_candidates (id INTEGER PRIMARY KEY AUTOINCREMENT, candidate TEXT, n INTEGER, frequency INTEGER, measure TEXT, value FLOAT)"
                )
                self.cur.execute(
                    "CREATE TABLE index_pt(id INTEGER PRIMARY KEY AUTOINCREMENT, source TEXT, target TEXT, probability FLOAT)"
                )
                self.cur.execute("CREATE INDEX index_index_pt on index_pt (source);")
                self.cur.execute(
                    "CREATE TABLE linguistic_patterns (id INTEGER PRIMARY KEY AUTOINCREMENT, linguistic_pattern TEXT)"
                )

                self.cur.execute(
                    "INSERT INTO configuration (id, sl_lang, tl_lang) VALUES (?,?,?)",
                    [0, self.sl_lang, self.tl_lang],
                )
                self.conn.commit()
    ...
    def open_project(self, project_name):
        ...
        if not os.path.isfile(project_name):
            ...
            with self.conn:
                ...
                """self.cur.execute('SELECT sl_pattern from sl_patterns')
                self.data=self.cur.fetchall()
                for self.d in self.data:
                    self.sl_patterns.append(self.d[0])
                    
                self.cur.execute('SELECT tl_pattern from tl_patterns')
                self.data=self.cur.fetchall()
                for self.d in self.data:
                    self.tl_patterns.append(self.d[0])"""
    ...
    def load_sl_corpus_s(self, corpus):
        ...
        self.data = []
        self.continserts = 0
        for self.line in corpus.split(os.linesep):
            self.continserts += 1
            ...
            if self.continserts == self.maxinserts:
                self.cur.executemany(
                    "INSERT INTO sl_corpus (id, segment) VALUES (?,?)", self.data
                )
                self.data = []
                self.continserts = 0
        with self.conn:
    ...
    def load_sl_tl_corpus(self, slcorpusfile, tlcorpusfile, encoding="utf-8"):
        ...
        while 1:
            ...
            if self.continserts == self.maxinserts:
                self.cur.executemany(
                    "INSERT INTO sl_corpus (id, segment) VALUES (?,?)", self.sl_data
                )
                self.cur.executemany(
                    "INSERT INTO tl_corpus (id, segment) VALUES (?,?)", self.tl_data
                )
                self.sl_data = []
                self.tl_data = []
                self.continserts = 0
    ...
    def load_tmx(self, tmx_file, sl_code=None, tl_code=None):
        ...
        for self.event, self.elem in etree.iterparse(tmx_file, events=("start", "end")):
            if self.event == "start":
                if (
                    self.elem.tag == "tu"
                    and not self.sl_segment == ""
                    and not self.tl_segment == ""
                ):
                    self.continserts += 1
                    self.max_id_corpus += 1
                    self.record1 = []
                    ...
                    if self.continserts == self.maxinserts:
                        self.cur.executemany(
                            "INSERT INTO sl_corpus (id, segment) VALUES (?,?)",
                            self.data1,
                        )
                        self.cur.executemany(
                            "INSERT INTO tl_corpus (id, segment) VALUES (?,?)",
                            self.data2,
                        )
                        self.data1 = []
                        self.data2 = []
                    ...
                    self.current_lang = self.elem.attrib[
    ...
    def load_sl_tagged_corpus(self, corpusfile, format="TBXTools", encoding="utf-8"):
        ...
        if format == "TBXTools":
            ...
            for self.line in self.cf:
                ...
                if self.continserts == self.maxinserts:
                    self.cur.executemany(
                        "INSERT INTO sl_tagged_corpus (id, tagged_segment) VALUES (?,?)",
                        self.data,
                    )
                    self.data = []
                    self.continserts = 0
            ...
            for self.line in self.cf:
                ...
                if self.line == "":
                    ...
                    self.token = (
                        self.camps[0] + "|" + self.camps[1] + "|" + self.camps[2]
                    )
            ...
            for self.line in self.cf:
                ...
                if self.line == "":
                    ...
                    self.camps = self.line.split("\t")
                    self.token = (
                        self.camps[1] + "|" + self.camps[2] + "|" + self.camps[3]
                    )
                    self.segment.append(self.token)
    ...
    def load_sl_stopwords(self, fitxer, encoding="utf-8"):
        ...
        with self.conn:
            self.cur.executemany(
                "INSERT INTO sl_stopwords (sl_stopword) VALUES (?)", self.data
            )
    ...
    def load_tl_stopwords(self, fitxer, encoding="utf-8"):
        ...
        with self.conn:
            self.cur.executemany(
                "INSERT INTO tl_stopwords (tl_stopword) VALUES (?)", self.data
            )
    ...
    def load_sl_inner_stopwords(self, fitxer, encoding="utf-8"):
        ...
        with self.conn:
            self.cur.executemany(
                "INSERT INTO sl_inner_stopwords (sl_inner_stopword) VALUES (?)",
                self.data,
            )
    ...
    def load_tl_inner_stopwords(self, fitxer, encoding="utf-8"):
        ...
        with self.conn:
            self.cur.executemany(
                "INSERT INTO tl_inner_stopwords (tl_inner_stopword) VALUES (?)",
                self.data,
            )
    ...
    def load_evaluation_terms(self, arxiu, encoding="utf-8", nmin=0, nmax=1000):
        ...
        for self.line in self.cf:
            ...
            if len(self.camps) == 1:
                if (
                    len(self.camps[0].split(" ")) >= nmin
                    and len(self.camps[0].split(" ")) <= nmax
                ):
        ...
        with self.conn:
            self.cur.executemany(
                "INSERT INTO evaluation_terms (sl_term,tl_term) VALUES (?,?)", self.data
            )
    ...
    def load_exclusion_terms(self, arxiu, encoding="utf-8", nmin=0, nmax=1000):
        ...
        for self.line in self.cf:
            ...
            if self.continserts == self.maxinserts:
                self.cur.executemany(
                    "INSERT INTO exclusion_terms (sl_term,tl_term) VALUES (?,?)",
                    self.data,
                )
                self.data = []
                self.continserts = 0
        with self.conn:
            self.cur.executemany(
                "INSERT INTO exclusion_terms (sl_term,tl_term) VALUES (?,?)", self.data
            )
    ...
    def load_sl_exclusion_regexps(self, arxiu, encoding="utf-8"):
        ...
        with self.conn:
            self.cur.executemany(
                "INSERT INTO sl_exclusion_regexps (sl_exclusion_regexp) VALUES (?)",
                self.data,
            )
    ...
    def load_tl_exclusion_regexps(self, arxiu, encoding="utf-8"):
        ...
        with self.conn:
            self.cur.executemany(
                "INSERT INTO tl_exclusion_regexps (sl_exclusion_regexp) VALUES (?)",
                self.data,
            )

    def show_term_candidates(
        self,
        limit=-1,
        minfreq=2,
        minmeasure=-1,
        ...
        with self.conn:
            ...
            for exclusion_term in self.cur.fetchall():
                self.knownterms.append(exclusion_term[0])
        with self.conn:
            self.cur.execute(
                "SELECT frequency,measure,n,candidate FROM term_candidates order by frequency desc limit "
                + str(limit)
            )
            for candidate_term in self.cur.fetchall():
                self.frequency = candidate_term[0]
                if candidate_term[1] == None:
                    self.measure == 0
                else:
                    self.measure = candidate_term[1]
                self.n = candidate_term[2]
                self.candidate = candidate_term[3]
                if (
                    self.n >= self.n_min
                    and self.n <= self.n_max
                    and not self.candidate in self.knownterms
                ):
                    if mark_eval:
    ...
    def select_unigrams(self, file, position=-1, verbose=True):
        ...
        self.unigrams = {}
        self.cur.execute(
            "SELECT frequency,candidate FROM term_candidates order by value desc, frequency desc, random()"
        )
        ...
        for s in self.cur.fetchall():
            self.frequency = s[0]
            self.candidate = s[1].split(" ")[position]
        ...
        for self.candidate in sorted(
            self.unigrams, key=self.unigrams.get, reverse=True
        ):
            if verbose:
    ...
    def generate_term_candidates(
        ...
        with self.conn:
            self.cur.execute(
                "SELECT frequency,value,n,candidate FROM term_candidates order by value desc, frequency desc, random() limit "
                + str(limit)
            )
            # self.cur.execute("SELECT frequency,value,n,candidate FROM term_candidates order by n desc limit "+str(limit))
            for candidate_term in self.cur.fetchall():
    ...
    def save_term_candidates(
        ...
        """Saves the term candidates in a file."""
        out = self.generate_term_candidates(
            limit, minfreq, minmeasure, show_frequency, show_measure, mark_eval, verbose
        )
        self.sortida = codecs.open(outfile, "w", encoding="utf-8")
        self.sortida.writelines(out)
        return out
    ...
    def ngram_calculation(self, nmin, nmax, minfreq=2):
        ...
        with self.conn:
            ...
            for self.s in self.cur.fetchall():
                ...
                for self.n in range(
                    nmin, nmax + 2
                ):  # we calculate one order bigger in order to detect nested candidates
        ...
        with self.conn:
            self.cur.executemany(
                "INSERT INTO ngrams (ngram, n, frequency) VALUES (?,?,?)", self.data
            )
            self.conn.commit()
        ...
        with self.conn:
            self.cur.executemany(
                "INSERT INTO tokens (token, frequency) VALUES (?,?)", self.data
            )
            self.conn.commit()
    ...
    def statistical_term_extraction(self, minfreq=2):
        ...
        self.conn.commit()
        self.cur.execute(
            "SELECT ngram, n, frequency FROM ngrams order by frequency desc"
        )
        ...
        for self.a in self.results:
            ...
            self.include = True
            if self.ng[0].lower() in self.sl_stopwords:
                self.include = False
            if self.ng[-1].lower() in self.sl_stopwords:
                self.include = False
        ...
        with self.conn:
            self.cur.executemany(
                "INSERT INTO term_candidates (candidate, n, frequency, measure, value) VALUES (?,?,?,?,?)",
                self.data,
            )
            self.conn.commit()
    ...
    def case_normalization(self, verbose=False):
        ...
        for self.a in self.results:
            if (
                not self.a[0] == self.a[0].lower()
                and self.a[0].lower() in self.auxiliar
            ):
                ...
                self.cur.execute(
                    "DELETE FROM term_candidates WHERE candidate=?", (self.terma,)
                )
                self.cur.execute(
                    "DELETE FROM term_candidates WHERE candidate=?", (self.termb,)
                )
                # self.cur.execute("INSERT INTO term_candidates (candidate, n, frequency) VALUES (?,?,?)",(self.termb,self.n,self.freqtotal,))
                self.cur.execute(
                    "INSERT INTO term_candidates (candidate, n, frequency, measure, value) VALUES (?,?,?,?,?)",
                    (self.termb, self.n, self.freqtotal, "freq", self.freqtotal),
                )
        self.conn.commit()
    ...
    def nest_normalization(self, percent=10, verbose=False):
        ...
        """
        self.cur.execute(
            "SELECT candidate,frequency,n FROM term_candidates order by frequency desc"
        )
        self.results = self.cur.fetchall()
        for self.a in self.results:
            ...
            self.cur.execute(
                "SELECT candidate,frequency FROM term_candidates where frequency <="
                + str(self.fmax)
                + " and frequency>="
                + str(self.fmin)
                + "  and n ="
            ...
            for self.b in self.results2:
                ...
                if not self.ta == self.tb and not self.tb.find(self.ta) == -1:
                    ...
                    self.cur.execute(
                        "DELETE FROM term_candidates WHERE candidate=?", (self.ta,)
                    )
                    if verbose:
                        pass
    ...
    def regexp_exclusion(self, verbose=False):
        ...
        for self.r in self.results:
            ...
            self.exreg = self.r[0]
            self.cur.execute(
                "SELECT candidate FROM term_candidates where n<=" + str(self.n_max)
            )
            self.results = self.cur.fetchall()
    ...
    def evaluate_pos(self, limit, order="desc", iterations=1000, ignore_case=True):
        ...
        with self.conn:
            for i in range(0, iterations):
                if order == "desc":
                    self.cur.execute(
                        "SELECT candidate from term_candidates where n<="
                        + str(self.n_max)
                        + " order by value desc, frequency desc, random() limit "
                        + str(limit)
                    )
                elif order == "asc":
                    self.cur.execute(
                        "SELECT candidate from term_candidates where n<="
                        + str(self.n_max)
                        + " order by value asc, frequency desc, random() limit "
                        + str(limit)
                    )
                else:
                    raise NameError(
                        "Order must be desc (decending) or asc (ascending). Defaulf value: desc"
                    )
    ...
    def association_measures(self, measure="raw_freq", n=2):
        ...
        if n == 2:
            ...
            if measure == "chi_sq":
                self.bigram_measure = self.bigram_finder.score_ngrams(
                    self.bigram_measures.chi_sq
                )
            elif measure == "chi_sq_2g":
                self.bigram_measure = self.bigram_finder.score_ngrams(
                    self.bigram_measures.chi_sq_2g
                )
            elif measure == "fisher":
                self.bigram_measure = self.bigram_finder.score_ngrams(
                ...
                self.bigram_measure = self.bigram_finder.score_ngrams(
                    self.bigram_measures.phi_sq
                )
            elif measure == "phi_sq_2g":
                self.bigram_measure = self.bigram_finder.score_ngrams(
                    self.bigram_measures.phi_sq_2g
                )
            elif measure == "dice":
                self.bigram_measure = self.bigram_finder.score_ngrams(
                    self.bigram_measures.dice
                )
            elif measure == "dice_2g":
                self.bigram_measure = self.bigram_finder.score_ngrams(
                    self.bigram_measures.dice_2g
                )
            elif measure == "my_dice":
                self.bigram_measure = self.bigram_finder.score_ngrams(
                    self.bigram_measures.my_dice
                )
            elif measure == "jaccard":
                self.bigram_measure = self.bigram_finder.score_ngrams(
                    self.bigram_measures.jaccard
                )
            elif measure == "jaccard_2g":
                self.bigram_measure = self.bigram_finder.score_ngrams(
                    self.bigram_measures.jaccard_2g
                )
                ...
                self.bigram_measure = self.bigram_finder.score_ngrams(
                    self.bigram_measures.loglikelihood_2g
                )
            elif measure == "pmi":
                self.bigram_measure = self.bigram_finder.score_ngrams(
                    self.bigram_measures.pmi
                )
            elif measure == "pmi_2g":
                self.bigram_measure = self.bigram_finder.score_ngrams(
                    self.bigram_measures.pmi_2g
                )
            elif measure == "poisson_stirling":
                self.bigram_measure = self.bigram_finder.score_ngrams(
                    self.bigram_measures.poisson_stirling
                )
            elif measure == "poisson_stirling_2g":
                self.bigram_measure = self.bigram_finder.score_ngrams(
                    self.bigram_measures.poisson_stirling_2g
                )
            elif measure == "student_t":
                self.bigram_measure = self.bigram_finder.score_ngrams(
                    self.bigram_measures.student_t
                )
            elif measure == "t_score_2g":
                self.bigram_measure = self.bigram_finder.score_ngrams(
                    self.bigram_measures.t_score_2g
                )
            elif measure == "mi_like":
                self.bigram_measure = self.bigram_finder.score_ngrams(
                    self.bigram_measures.mi_like
                )
            elif measure == "raw_freq":
                self.bigram_measure = self.bigram_finder.score_ngrams(
                    self.bigram_measures.raw_freq
                )

            elif measure == "tmi_2g":
                self.bigram_measure = self.bigram_finder.score_ngrams(
                    self.bigram_measures.tmi_2g
                )

            elif measure == "odds_2g":
                self.bigram_measure = self.bigram_finder.score_ngrams(
                    self.bigram_measures.odds_2g
                )

            elif measure == "MI":
                self.bigram_measure = self.bigram_finder.score_ngrams(
                    self.bigram_measures.MI
                )
            elif measure == "MI2":
                self.bigram_measure = self.bigram_finder.score_ngrams(
                    self.bigram_measures.MI2
                )
            elif measure == "MI3":
                self.bigram_measure = self.bigram_finder.score_ngrams(
                    self.bigram_measures.MI3
                )
                ...
                self.bigram_measure = self.bigram_finder.score_ngrams(
                    self.bigram_measures.t_score
                )
            elif measure == "log_likelihood_ratio":
                self.bigram_measure = self.bigram_finder.score_ngrams(
                    self.bigram_measures.log_likelihood_ratio
                )

            elif measure == "log_likelihood_ratio_TEXTNSP":
                self.bigram_measure = self.bigram_finder.score_ngrams(
                    self.bigram_measures.log_likelihood_ratio_TEXTNSP
                )
            elif measure == "tmi":
                self.bigram_measure = self.bigram_finder.score_ngrams(
                    self.bigram_measures.tmi
                )

            elif measure == "poisson_stirling_TEXTNSP":
                self.bigram_measure = self.bigram_finder.score_ngrams(
                    self.bigram_measures.poisson_stirling_TEXTNSP
                )

            elif measure == "chi_squared":
                self.bigram_measure = self.bigram_finder.score_ngrams(
                    self.bigram_measures.chi_squared
                )

            elif measure == "jaccard_TEXTNSP":
                self.bigram_measure = self.bigram_finder.score_ngrams(
                    self.bigram_measures.jaccard_TEXTNSP
                )

            elif measure == "pmi_TEXTNSP":
                self.bigram_measure = self.bigram_finder.score_ngrams(
                    self.bigram_measures.pmi_TEXTNSP
                )

            elif measure == "phi_sq_TEXTNSP":
                self.bigram_measure = self.bigram_finder.score_ngrams(
                    self.bigram_measures.phi_sq_TEXTNSP
                )
            ...
            if self.measurename == "chi_sq":
                self.trigram_measure = self.trigram_finder.score_ngrams(
                    self.trigram_measures.chi_sq
                )

            elif self.measurename == "jaccard":
                self.trigram_measure = self.trigram_finder.score_ngrams(
                    self.trigram_measures.jaccard
                )
                ...
                self.trigram_measure = self.trigram_finder.score_ngrams(
                    self.trigram_measures.pmi
                )
            elif self.measurename == "poisson_stirling":
                self.trigram_measure = self.trigram_finder.score_ngrams(
                    self.trigram_measures.poisson_stirling
                )
            elif self.measurename == "student_t":
                self.trigram_measure = self.trigram_finder.score_ngrams(
                    self.trigram_measures.student_t
                )
            elif self.measurename == "mi_like":
                self.trigram_measure = self.trigram_finder.score_ngrams(
                    self.trigram_measures.mi_like
                )
            elif self.measurename == "raw_freq":
                self.trigram_measure = self.trigram_finder.score_ngrams(
                    self.trigram_measures.raw_freq
                )

            elif measure == "pmi_3g":
                self.trigram_measure = self.trigram_finder.score_ngrams(
                    self.trigram_measures.pmi_3g
                )

            elif measure == "poisson_stirling_3g":
                self.trigram_measure = self.trigram_finder.score_ngrams(
                    self.trigram_measures.poisson_stirling_3g
                )

            elif measure == "tmi_3g":
                self.trigram_measure = self.trigram_finder.score_ngrams(
                    self.trigram_measures.tmi_3g
                )

            elif measure == "loglikelihood_3g":
                self.trigram_measure = self.trigram_finder.score_ngrams(
                    self.trigram_measures.loglikelihood_3g
                )
            else:
                raise NameError("Association measure not implemented")
            self.data = []
        ...
        self.conn.executemany(
            "UPDATE term_candidates SET measure=?,value=? where candidate=?", self.data
        )
        self.conn.commit()
    ...
    def index_phrase_table(self, phrasetable):
        ...
        while 1:
            ...
            if (
                not self.trad[0] in self.punctuation
                and not self.source[0] in self.punctuation
                and not self.trad[-1] in self.punctuation
                and not self.source[-1] in self.punctuation
            ):
                ...
                if self.continserts == self.maxinserts:
                    self.cur.executemany(
                        "INSERT INTO index_pt (source, target, probability) VALUES (?,?,?)",
                        self.data,
                    )
                    self.data = []
                    self.continserts = 0
    ...
    def find_translation_ptable(
        self, sourceterm, maxdec=1, maxinc=1, ncandidates=5, separator=":"
    ):
        ...
        self.cur.execute(
            "SELECT target,probability FROM index_pt where source =?", (sourceterm,)
        )
        self.results = self.cur.fetchall()
        ...
        for self.a in self.results:
            ...
            if (
                not self.tttokens[0] in self.tl_stopwords
                and not self.tttokens[-1] in self.tl_stopwords
                and len(self.tttokens) >= len(sourceterm.split(" ")) - maxdec
                and len(self.tttokens) <= len(sourceterm.split(" ")) + maxinc
            ):
                self.targetcandidates[self.targetterm] = self.probability
        self.sorted_x = sorted(
            self.targetcandidates.items(), key=operator.itemgetter(1), reverse=True
        )
        self.results = []
    ...
    def start_freeling_api(self, LANG):
        ...
        self.mf1.set_active_options(
            False,
            True,
            True,
            False,  # select which among created
            True,
            True,
            False,
        ...
        with self.conn:
            ...
            for self.s in self.cur.fetchall():
                ...
                if self.continserts == self.maxinserts:
                    ...
                    if corpus == "target":
                        self.cur.executemany(
                            "INSERT INTO tl_tagged_corpus (id, tagged_segment) VALUES (?,?)",
                            self.data,
                        )
                    self.data = []
                    self.continserts = 0
    ...
    def tagged_ngram_calculation(self, nmin=2, nmax=3, minfreq=2):
        ...
        with self.conn:
            self.cur.executemany(
                "INSERT INTO tagged_ngrams (ngram, tagged_ngram, n, frequency) VALUES (?,?,?,?)",
                self.data,
            )
            self.conn.commit()
    ...
    def translate_linguistic_pattern(self, pattern):
        ...
        for self.ptoken in pattern.split(" "):
            self.auxtoken = []
            self.ptoken = self.ptoken.replace(
                ".*", "[^\s]+"
            )  # ATENCIÓ AIXÓ ÉS NOU, VERIFICAR SI CAL
    ...
    def load_linguistic_patterns(self, file, encoding="utf-8"):
        ...
        with self.conn:
            self.cur.executemany(
                "INSERT INTO linguistic_patterns (linguistic_pattern) VALUES (?)",
                self.data,
            )
    ...
    def linguistic_term_extraction(self, minfreq=2):
        ...
        self.cur.execute(
            "SELECT tagged_ngram, n, frequency FROM tagged_ngrams order by frequency desc"
        )
        self.results = self.cur.fetchall()
        self.data = []
        ...
        for self.a in self.results:
            ...
            if self.ng.split(" ")[0].split("|")[1].lower() in self.sl_stopwords:
                self.include = False
            if self.ng.split(" ")[-1].split("|")[1].lower() in self.sl_stopwords:
                self.include = False
    ...
    def learn_linguistic_patterns(
        ...
        with self.conn:
            ...
            for self.s in self.cur.fetchall():
                self.cur.execute(
                    "SELECT tagged_ngram, n, frequency FROM tagged_ngrams WHERE ngram= ?",
                    (self.s[0],),
                )
                self.results = self.cur.fetchall()
                ...
                if len(self.results) > 0:
                    for self.a in self.results:
                        ...
                        if self.pattern in self.learntpatterns:
                            ...
                            self.acufreq += self.n
        self.sorted_x = sorted(
            self.learntpatterns.items(), key=operator.itemgetter(1), reverse=True
        )
        self.results = []
        self.acufreq2 = 0
        for self.s in self.sorted_x:
    ...
    def find_translation_pcorpus(
        self, slterm, maxdec=1, maxinc=1, ncandidates=5, separator=":"
    ):
        ...
        with self.conn:
            ...
            for self.s in self.cur.fetchall():
                ...
                if self.segment.find(slterm) > -1:
                    self.cur2.execute(
                        'SELECT segment from tl_corpus where id="' + str(self.id) + '"'
                    )
                    for self.s2 in self.cur2.fetchall():
                        ...
                        for self.n in range(self.nmin, self.nmax + 1):
                            ...
                            for self.tlng in self.tlngs:
                                if (
                                    not self.tlng[0] in self.tl_stopwords
                                    and not self.tlng[-1] in self.tl_stopwords
                                ):
    ...
    def tsr(
        self, termsfile, encoding="utf-8", type="strict", max_iterations=10000000000
        ...
        while self.new:
            ...
            self.new = False
            self.cur.execute(
                "SELECT candidate,n,frequency,measure,value FROM term_candidates"
            )
            self.results = self.cur.fetchall()
            self.auxiliar = {}
            for self.r in self.results:
                ...
                if type == "strict":
                    if (
                        self.rcamps[0] in self.firstcomponent
                        and self.rcamps[-1] in self.lastcomponent
                    ):
                    ...
                    if (
                        self.rcamps[0] in self.firstcomponent
                        or self.rcamps[-1] in self.lastcomponent
                    ):
                        if not self.candidate in self.newcandidates:
                            self.newcandidates[self.candidate] = self.frequency
                            self.hashmeasure[self.candidate] = self.measure
                    ...
                    if self.iterations == 1:
                        ...
                        if (
                            ...
                            if not self.candidate in self.newcandidates:
                                self.newcandidatestempcombined[
                                    self.candidate
                                ] = self.frequency
                                self.hashmeasuretempcombined[
                                    self.candidate
...
class myBigramAssocMeasures(nltk.collocations.BigramAssocMeasures):
    ...
    def loglikelihood_2g(self, n_ii, n_ix_xi_tuple, n_xx):
        ...
        try:
            LogLikelihood = 2 * (
                n11 * math.log((n11 / m11), 2)
                + n12 * math.log((n12 / m12), 2)
                + n21 * math.log((n21 / m21), 2)
                + n22 * math.log((n22 / m22), 2)
            )
        except:
            LogLikelihood = 0
        ...
        try:
            tmi = (
                ...
                * math.log(
                ...
                + (n12 / npp) * math.log((n12 / m12), 2)
                + (n21 / npp) * math.log((n21 / m21), 2)
                + (n22 / npp) * math.log((n22 / m22), 2)
            )
        except:
            tmi = 0
        return tmi
    ...
    def log_likelihood_ratio_TEXTNSP(self, n_ii, n_ix_xi_tuple, n_xx):
        ...
        try:
            self.LLR = 2 * (
                (n_ii * math.log((n_ii / m11), 2))
                + (n_oi * math.log((n_oi / m12), 2))
                + (n_io * math.log((n_io / m21), 2))
                + (n_oo * math.log((n_oo / m22), 2))
            )
        except:
            self.LRR = 0
    ...
    def chi_squared(self, n_ii, n_ix_xi_tuple, n_xx):
        ...
        phi2 = ((n_ii * n_oo) - (n_io * n_io)) ** 2 / (
            n_xi * n_ix * (n_oi + n_oo) * (n_io + n_oo)
        )
    ...
    def phi_sq_TEXTNSP(self, n_ii, n_ix_xi_tuple, n_xx):
        ...
        self.phi2 = ((n_ii * n_oo) - (n_io * n_io)) ** 2 / (
            n_xi * n_ix * (n_oi + n_oo) * (n_io + n_oo)
        )
...
class myTrigramAssocMeasures(nltk.collocations.TrigramAssocMeasures):
    ...
    def tmi_3g(cls, *dades):
        ...
        try:
            tmi = (
                n111 / nppp * math.log((n111 / m111), 2)
                + n112 / nppp * math.log((n112 / m112), 2)
                + n121 / nppp * math.log((n121 / m121), 2)
                + n122 / nppp * math.log((n122 / m122), 2)
                + n211 / nppp * math.log((n211 / m211), 2)
                + n212 / nppp * math.log((n212 / m212), 2)
                + n221 / nppp * math.log((n221 / m221), 2)
                + n222 / nppp * math.log((n222 / m222), 2)
            )
        except:
            tmi = 0
    ...
    def loglikelihood_3g(cls, *dades):
        ...
        try:
            Log_Likelihood = 2 * (
                n111 * math.log((n111 / m111), 2)
                + n112 * math.log((n112 / m112), 2)
                + n121 * math.log((n121 / m121), 2)
                + n122 * math.log((n122 / m122), 2)
                + n211 * math.log((n211 / m211), 2)
                + n212 * math.log((n212 / m212), 2)
                + n221 * math.log((n221 / m221), 2)
                + n222 * math.log((n222 / m222), 2)
            )
        except:
            Log_Likelihood = 0
